<template>
  <div class="dashboard-container">
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
     ...
     <el-form-item label="文章状态">
          <el-select v-model="formInline.status" placeholder="请选择" >
              <el-option key="01" label="发布" value="01"></el-option>   
              <el-option key="02" label="草稿" value="02"></el-option>    
          </el-select> 
      </el-form-item>
      <el-form-item>
    ...
    <div class="leftPage">
      <el-button type="text" @click="dialogFormVisible">添加文章内容</el-button>
    </div>
      <el-table
      class="tableHeight"
...
import {httpRequest4Post} from '@/utils/publicMethod'

import { getInfoList,delInfoById } from '@/api/articleContent'
  ...
  methods:{
    ...
    onDel(row){
      ...
      this.$confirm('是否进行该类型删除？', '确认删除', {
          ...
          .then(() => { //成功
            delInfoById(params).then(response => {
              if(response.code == 1){
                _this.getInfo();
              }else {
                _this.tableData  = [];
    ...
    dialogFormVisible(){
      //Note : 文章内容 不适合嵌套页面开发，所以另起一个页面指向行
